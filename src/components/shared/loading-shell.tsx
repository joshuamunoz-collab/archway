import { Skeleton } from '@/components/ui/skeleton'

/** Static sidebar shell used in loading.tsx files (no auth needed) */
export function LoadingShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-background">
      {/* Static sidebar placeholder */}
      <aside
        className="fixed inset-y-0 left-0 z-40 w-60 flex flex-col"
        style={{ backgroundColor: '#1A1A2E' }}
      >
        <div className="h-14 flex items-center px-5 shrink-0">
          <span className="text-base font-semibold tracking-tight" style={{ color: '#FFFFFF' }}>Archway</span>
        </div>
        <div style={{ height: 1, backgroundColor: 'rgba(255,255,255,0.1)' }} />
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-1">
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="flex items-center gap-3 rounded-md px-3 py-2">
              <Skeleton className="h-4 w-4 shrink-0 opacity-10" />
              <Skeleton className="h-4 flex-1 opacity-10" />
            </div>
          ))}
        </nav>
        <div style={{ height: 1, backgroundColor: 'rgba(255,255,255,0.1)' }} />
        <div className="p-3 shrink-0">
          <div className="flex items-center gap-2 px-2 py-1.5">
            <Skeleton className="h-7 w-7 rounded-full opacity-10" />
            <div className="flex-1 space-y-1">
              <Skeleton className="h-3 w-24 opacity-10" />
              <Skeleton className="h-3 w-32 opacity-10" />
            </div>
          </div>
        </div>
      </aside>
      <div className="ml-60 min-h-screen flex flex-col">
        {/* Top header placeholder matching AppShell */}
        <header className="sticky top-0 z-30 h-14 shrink-0 border-b bg-white/80 backdrop-blur-sm flex items-center px-6 gap-4">
          <Skeleton className="h-9 w-full max-w-md rounded-md" />
        </header>
        <main className="flex-1">
          {children}
        </main>
      </div>
    </div>
  )
}
